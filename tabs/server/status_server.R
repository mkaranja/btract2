...
################################################################################################## 
  ...
  output$overviewPlot <- renderCollapsibleTree({
    ...
    result = cleantable() %>%
      dplyr::filter(!Activity %in% c('Flowering', 'Status', 'Repeat pollination') & 
                      Date >= input$df_dateRange[1] & Date <= input$df_dateRange[2])
  ...
  output$schedulerOut <- renderUI({
    div(
      ...
      if(nrow(overdue())==0 & nrow(ready())==0 & nrow(approaching())==0){
        div(br(), br(), br(),
          tags$p(style = "color: #FF8C00; font-size: 18px; text-align: center;", "Nothing To Show"),
          tags$p(style = "color: green; font-size: 20px; text-align: center;", "Your Records Are On Time")
        )
      }
    )
  })
